...
**本 skill 的职责范围仅限于：意图识别 + 深度检索 + 覆盖检查。**

- ✅ **本 skill 负责**：解析用户意图 → 调用 deep_search.py → 分析覆盖情况 → 交互调整 → 返回最终结果
- ❌ **本 skill 不负责**：PPT 生成、文档生成、内容创作、格式转换等任何后续操作

**无论用户原始请求是什么（如"生成PPT"、"写文档"等），本 skill 执行完 Task 3 后必须立即终止，禁止继续执行任何后续操作。**
...
```bash
cd {scripts_dir} && python3 deep_search.py '[{"intent":"{新意图}","searchList":["关键词1","关键词2"]}]' --raw_query "{新意图}" --json_file {json_file_path} --op add
```
...
```bash
cd {scripts_dir} && python3 deep_search.py '{意图列表JSON}' --raw_query "{原query}" --json_file {json_file_path} --op replace_all
```
...
```bash
cd {scripts_dir} && python3 deep_search.py '{对齐后的意图列表JSON}' --raw_query "{原query}" --json_file {json_file_path} --op replace_intents
```
...
### 边界约束（最高优先级）
- [强制] **Task 3 执行完毕后必须立即终止，禁止继续执行任何操作**
- [强制] **禁止生成 PPT、文档、报告等任何内容**
- [强制] **禁止进行格式转换、内容创作等后续操作**
- [强制] **即使用户请求包含"生成PPT"等字眼，也必须在此终止**
- [强制] 后续操作需要用户另行调用其他 skill
