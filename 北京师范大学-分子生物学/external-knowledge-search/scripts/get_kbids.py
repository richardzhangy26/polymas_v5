...
def main():
    ...
    if DEFAULT_USER_ID:
        ...
        kb_lists = [{"kbId":res.get("kbId", ""),"kbName":res.get("kbName", "")} for res in api_result]

        print(json.dumps({"apps": kb_lists}, ensure_ascii=False))
        sys.exit(0)
    else:
        print(json.dumps({"apps": {"knowledgeBases":""}}, ensure_ascii=False))
        sys.exit(0)
