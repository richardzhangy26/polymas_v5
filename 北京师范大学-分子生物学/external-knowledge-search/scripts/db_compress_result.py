#!/usr/bin/env python3
"""
compress_result.py — db-search 结果压缩模块

功能：
1. 按相似度排序，总 token 控制在阈值内（默认 96K）
...
DEFAULT_CONFIG = {
    ...
    "overflow_dir": os.path.join(tempfile.gettempdir(), "openclaw-overflow", "db-search"),
...
def format_single_result(item, index, link_map, counter, is_full, config):
    num = index + 1
    similarity = item.get("similarity") or item.get("score") or 0
    sim_percent = f"{similarity * 100:.1f}"

    title = item.get("title") or item.get("fileName") or f"结果 {num}"
    title = replace_markdown_links(title, link_map, counter)

    source_link = ""
    url = item.get("previewUrl") or item.get("url") or item.get("src")
    if url:
        counter["count"] += 1
        lid = f"L{counter['count']}"
        link_map[lid] = {
    ...
    meta.append(f"📊 相似度 {sim_percent}%")

    content = item.get("content") or item.get("description") or item.get("text") or ""
    content = replace_markdown_links(content, link_map, counter)
    ...
    overflow_md = f"### [{num}] {title} (相似度 {sim_percent}%)\n\n{content}\n\n"
...
def compress_result(raw_result, options=None):
    ...
    results = list(raw_result.get("results") or [])
    results.sort(key=lambda x: x.get("similarity") or x.get("score") or 0, reverse=True)
    ...
    markdown = f"## 📚 知识库检索结果\n\n> 共 **{len(results)}** 条结果，按相似度排列（前 {kept_full} 条完整，{kept_summary} 条摘要）\n\n"
